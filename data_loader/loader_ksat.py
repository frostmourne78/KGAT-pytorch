...
def remap_item(train_data, test_data):
    ...
    for u_id, i_id in train_data:
        train_user_set[int(u_id)].append(int(i_id))  #remap 交互数据
...
def build_graph(train_data, triplets):
    ...
    print("\nBegin to load knowledge graph triples ...")#依据关系来划分 将相同关系的h和t放在一块
...
def build_sparse_relational_graph(relation_dict):
    def _bi_norm_lap(adj): #对称标准化 （symmetric normalized Laplacian）
        # D^{-1/2}AD^{-1/2}
    ...
    def _si_norm_lap(adj):   #随机游走标准化 (random walk Laplacian)
        # D^{-1}A
        rowsum = np.array(adj.sum(1))
...
def load_data(model_args):
    global args
    args = model_args
    directory = args.data_dir + args.dataset + '/'
